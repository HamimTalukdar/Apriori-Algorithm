...
# Give the CSV file path location
df = pd.read_csv('ML/Dataset/basket_analysis.csv')

list_type = list(df.columns[1:])
print('Columns:', list_type)
#This is Hamim Talukdar...
...
for i in range(len(df)):
    ...
    for j in range(2):
        if j==0:
            ...
            # Read each Row and convert them into set
            l=set(list_type[index] for index,num in enumerate(df.iloc[i][1:17]) if num==True)
            mset.update(l)
            lp.append(l)
